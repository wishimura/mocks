...
- **PR作成後 → 確認なしでそのまま squash マージする**（ユーザー指示・毎回）
  - これはあくまでモック集なので、都度の承認は不要
  - コンフリクトが出たら base ブランチを取り込んで解消してからマージする
...
| ディレクトリ | 対象 | 特徴 |
|--------------|------|------|
| `citrus/` | Citrus Salon（酵素風呂サロン） | ウッディブラウン+セージグリーン、回数券デジタル管理・LINE教育配信でLTV向上（POS連携は将来課題） |
